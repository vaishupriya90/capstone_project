...
import getPaintings from './painting-actions';
...
test('dispatch GET_PAINTINGS when getPaintings is called', async () => {
  const mockApi = new MockAdapter(axios);
  mockApi.onGet(`${process.env.REACT_APP_BASE_API}/api/paintings`).reply(200, []);
  getPaintings();
});
